...
object Example extends App {
  ...
  keystores.aliases().asScala.map(keystores.getCertificateChain).foreach(x => println(x.length))
  keystores.aliases().asScala.map(keystores.getCertificateChain).foreach(x => println(x(0)))
